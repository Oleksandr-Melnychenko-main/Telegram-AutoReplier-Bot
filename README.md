...
An OpenAI API Key (get this from the OpenAI Developer Platform).
...
You should see Bot's running in your terminal.
...
⚠️ Notes
Make sure to disable Privacy Mode via BotFather if you want the bot to read all group messages (though currently, it is strictly filtered to only process explicit mentions and replies).

The current AI model is set to gpt-4o-mini with a temperature of 0.7. This can be adjusted inside services/ai_service.py.
